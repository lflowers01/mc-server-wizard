...
def latest_mc_version():
    return str(requests.get("https://launchermeta.mojang.com/mc/game/version_manifest.json").json()["latest"]["release"])
...
class Paper:
    ...
    def latest_build(self):
        return requests.get(f"https://papermc.io/api/v2/projects/paper/versions/{self.version}").json()['builds'][-1]
...
class Vanilla:
    ...
    def get_url(self):
        version_list = requests.get("https://launchermeta.mojang.com/mc/game/version_manifest.json").json()['versions']
...
class Pufferfish:
    ...
    def __init__(self, version: str):
        ...
        if self.intermidiate_version < 17:
            raise ValueError(f"Pufferfish is only available for 1.17.X and above.")
        self.url = self.get_file()

    def get_file(self):
        builds = requests.get(f"https://ci.pufferfish.host/job/Pufferfish-1.{self.intermidiate_version}/api/json").json()["builds"]
        for build in builds:
            v = requests.get(f"{build['url']}/api/json").json()['artifacts'][0]['displayPath']
